...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signupUser.fulfilled.match(result)) {
      toast({
        ...
        description: "Welcome to our cosmetics store!",
      });
      navigate(`/?id=${result.payload.id}`);
    }
  };
  ...
  const passwordStrength = () => {
    ...
    if (password.length < 4) return { strength: 25, color: 'bg-red-500', text: 'Weak' };
    if (password.length < 8) return { strength: 50, color: 'bg-yellow-500', text: 'Fair' };
    if (password.length < 12) return { strength: 75, color: 'bg-blue-500', text: 'Good' };
        ...
        <Card className="shadow-2xl border-0 bg-white/80 backdrop-blur-sm">
          <CardHeader className="text-center space-y-2">
            ...
            <CardDescription className="text-gray-600">
              Join us and discover amazing cosmetics
            </CardDescription>
          </CardHeader>
          <CardContent>
